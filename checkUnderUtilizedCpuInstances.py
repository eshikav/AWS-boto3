...
class checkUnderUtilizedCpuInstances:
  '''
  Description:
  ...
  '''
  Description:
  getInstanceCpuUtilization gets the cpu utilizations of the instances
  ...
  '''
  def getInstanceCpuUtilization(self):
    try:
      ...
      for region in instances.keys():
        for index,instance in enumerate(instances[region]):
          ...
          if len(response['Datapoints']) != 0:
            instances[region][index]['cpu_utilization'] = self.getAverageDatapoints(response['Datapoints'])
          else:
            instances[region][index]['cpu_utilization'] = 0 
      return instances
    except Exception as e:
      returnFailjson(str(e))
  ...
  '''
  Description:
  getAverageDatapoints calculates the average of all the datapoints which are requested from the getInstanceCpuUtilization.
  ...
  def mark(self):
    try:
      instances = self.getInstanceCpuUtilization()
      for region in instances.keys():
        mark_tag = [{
                 'Key': 'Mark',
        ...
        for instance in instances[region]:
          if instance['Instances'][0]['InstanceId'] not in self.rule_properties['IgnoreResourceList']:
            if (instance['cpu_utilization'] < 10 and 'Tags' in instance['Instances'][0].keys() and 'Mark' in list(tag['Key'] for tag in instance['Instances'][0]['Tags'])):
              pass
            else:
  ...
  '''
  Description:
  checkUnderUtilizedCpuInstances reports all the instances which deviates from a specific rule.
  ...
  '''
  def checkUnderUtilizedCpuInstances(self): 
    try:
      instances = self.getInstanceCpuUtilization()
      result = True
      failReason = ""
      offenders = []
      control = "x.x"
      description = "Ensure there are no Instances with Unused CPU Utilization"
      scored = True
      counter = 0
      ...
      for region in instances.keys():
        for instance in instances[region]:
          if instance['Instances'][0]['InstanceId'] not in self.rule_properties['IgnoreResourceList']:
            ...
            if instance['Instances'][0]['InstanceId'] in self.rule_properties['IgnoreResourceList']:
              ignoredresources += 1
              ignored.append(dict({'region': region,'id': instance['Instances'][0]['InstanceId']}))
      if counter != 0:
